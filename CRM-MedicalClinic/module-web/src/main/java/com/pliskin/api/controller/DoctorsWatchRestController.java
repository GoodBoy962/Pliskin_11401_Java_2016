...
public class DoctorsWatchRestController extends BaseApiController {
    ...
    public ResponseEntity<List<Doctor>> getDoctors(@PathVariable("id") Long id,
    ...
    public ResponseEntity<List<DoctorSchedule>> getDoctorTimeTable(@PathVariable("id") Long id) {
        Doctor doctor = doctorService.getDoctor(id);
        if (doctor == null) {
            return createBadResponse(HttpStatus.NOT_FOUND);
        }
